...
const SCREENER_PRESETS: ScreenerPreset[] = [
  {
    ...
    description: "Price near 52W high + above-avg volume + ADX trend strength + MACD momentum",
    conditions: [
      { metric: "rsi_14",            op: "gte", value: 55 },
      { metric: "volume_ratio",      op: "gt",  value: 0.9 },
      { metric: "adx_14",            op: "gt",  value: 20 },
      { metric: "pct_from_52w_high", op: "gt",  value: -15 },
      { metric: "macd_hist",         op: "gt",  value: 0 },
    ],
    filters: [
      { metric: "rsi_14",            label: "RSI 14 ≥ 55",              why: "Momentum building, not yet overbought. RSI < 50 = no trend." },
      { metric: "volume_ratio",      label: "Volume Ratio > 0.9×",      why: "Near or above own avg — on NSE daily data, 0.9× already filters low-interest days." },
      { metric: "adx_14",            label: "ADX 14 > 20",              why: "Trend strength confirmed. ADX < 20 = choppy range, not a real breakout." },
      { metric: "pct_from_52w_high", label: "Within 15% of 52W High",  why: "Near breakout zone. NSE correction cycles often pull back 10-15% before the next leg." },
      { metric: "macd_hist",         label: "MACD Histogram > 0",      why: "Fresh momentum shift. Better leading signal than MACD line which lags." },
    ],
  },
  {
    id: "buy_dip", label: "Buy the Dip", icon: TrendingDown,
    description: "Oversold pullback within uptrend + institutional holding + profitable company",
    conditions: [
      { metric: "rsi_14",            op: "lt",  value: 42 },
      { metric: "stoch_k",           op: "lt",  value: 35 },
      { metric: "pct_from_52w_low",  op: "gt",  value: 15 },
      { metric: "delivery_pct",      op: "gt",  value: 40 },
      { metric: "eps_basic",         op: "gt",  value: 0 },
    ],
    filters: [
      { metric: "rsi_14",           label: "RSI 14 < 42",           why: "Approaching oversold. Aggressive sellers in control — pullback in progress." },
      { metric: "stoch_k",          label: "Stoch %K < 35",         why: "Second oscillator confirmation. RSI + Stoch both weak = selling pressure confirmed." },
  ...
  },
  {
    ...
    description: "Above-average volume + high delivery (institutional) + money flowing in (CMF) + price rising",
    conditions: [
      { metric: "volume_ratio",  op: "gt",  value: 1.0 },
      { metric: "delivery_pct", op: "gt",  value: 45 },
      { metric: "cmf_20",       op: "gt",  value: 0 },
      { metric: "change_1d",    op: "gt",  value: 0.3 },
      { metric: "rsi_14",       op: "gte", value: 40 },
    ],
    filters: [
      { metric: "volume_ratio",  label: "Volume Ratio > 1×",     why: "Above own 20-day avg — confirms unusual interest. On NSE daily, 1× is a meaningful threshold." },
      { metric: "delivery_pct", label: "Delivery % > 45%",       why: "Significant delivery component = institutional interest, not pure intraday trading." },
      { metric: "cmf_20",       label: "CMF 20 > 0",             why: "Chaikin Money Flow positive = net buying pressure over 20 days." },
      { metric: "change_1d",    label: "1D Change > 0.3%",       why: "Price moving up WITH volume = accumulation. Flat/down price with volume = distribution." },
      { metric: "rsi_14",       label: "RSI 14 ≥ 40",            why: "Not deeply oversold — accumulation happens in base or early uptrend, not crash territory." },
    ],
  },
  {
    id: "mean_reversal", label: "Mean Reversal", icon: RefreshCw,
    description: "Oversold across multiple oscillators + price at BB lower band + weak trend (ADX)",
    conditions: [
      { metric: "rsi_14",           op: "lt", value: 35 },
      { metric: "bb_pct",           op: "lt", value: 0.25 },
      { metric: "stoch_k",          op: "lt", value: 25 },
      { metric: "pct_from_52w_low", op: "gt", value: 15 },
      { metric: "adx_14",           op: "lt", value: 28 },
    ],
    filters: [
      { metric: "rsi_14",           label: "RSI 14 < 35",              why: "Solid oversold signal. NSE stocks below 35 RSI historically show above-avg reversal rates." },
      { metric: "bb_pct",           label: "BB %B < 0.25",             why: "Price in lower quarter of Bollinger Band = approaching statistical extreme." },
      { metric: "stoch_k",          label: "Stoch %K < 25",            why: "Triple confirmation. RSI + Stoch + BB all oversold = strong mean reversion setup." },
      { metric: "pct_from_52w_low", label: "> 15% from 52W Low",       why: "Had prior strength. At 52W lows with oversold signals = downtrend, not reversal." },
      { metric: "adx_14",           label: "ADX 14 < 28",              why: "Trend is weak. Strong downtrends (ADX > 28) usually continue — they don't snap back." },
    ],
  },
  {
    id: "quality_value", label: "Quality & Value", icon: Star,
    description: "Reasonable PE for NSE + low debt + profitable + promoter-backed + price moving up",
    conditions: [
      { metric: "pe_ratio",       op: "lt", value: 25 },
      { metric: "debt_to_equity", op: "lt", value: 1.0 },
      { metric: "eps_basic",      op: "gt", value: 3 },
      { metric: "promoter_pct",   op: "gt", value: 50 },
      { metric: "roc_20",         op: "gt", value: 0 },
    ],
    filters: [
      { metric: "pe_ratio",       label: "P/E < 25",           why: "Below NSE market average PE of ~22-25. Reasonable value by Indian market standards." },
      { metric: "debt_to_equity", label: "D/E < 1.0",          why: "Debt within industry norms. D/E > 1 starts increasing financial risk in rate cycles." },
      { metric: "eps_basic",      label: "EPS > ₹3",           why: "Meaningfully profitable. Filters out penny-profit companies with no real earnings power." },
      { metric: "promoter_pct",   label: "Promoter > 50%",     why: "Majority promoter holding = aligned management, lower dilution risk." },
      { metric: "roc_20",         label: "ROC 20D > 0%",       why: "Price up over 20 days = market agreeing. Value without any price action = value trap." },
    ],
  },
];
