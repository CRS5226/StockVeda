...
export default function ScreenerFilters() {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Presets */}
      <div>
        <div className="text-xs text-slate-400 font-medium mb-2">Presets</div>
        <div className="flex flex-col gap-1">
          {presets.map((p) => (
            <button key={p.name} onClick={() => setConditions(p.conditions)}
              className="text-left text-xs px-2 py-1.5 rounded bg-slate-100 hover:bg-slate-200 text-slate-600">
              {p.name}
            </button>
          ))}
        </div>
      ...
      {/* Conditions */}
      <div>
        <div className="text-xs text-slate-400 font-medium mb-2">Conditions</div>
        <div className="flex flex-col gap-2">
          {conditions.map((c, i) => (
            <div key={i} className="flex items-center gap-1">
              <select value={c.metric}
                onChange={(e) => updateCondition(i, { ...c, metric: e.target.value })}
                className="flex-1 bg-slate-100 text-slate-700 text-xs rounded px-1.5 py-1 outline-none">
                {metrics.map((m) => <option key={m} value={m}>{m}</option>)}
              </select>
              <select value={c.op}
                onChange={(e) => updateCondition(i, { ...c, op: e.target.value })}
                className="w-12 bg-slate-100 text-slate-700 text-xs rounded px-1.5 py-1 outline-none">
              ...
              <input type="number" value={c.value}
                ...
                className="w-16 bg-slate-100 text-slate-700 text-xs rounded px-1.5 py-1 outline-none" />
        ...
        <button
          ...
          className="mt-2 text-xs text-blue-500 hover:text-blue-300">
          + Add condition
        </button>
      </div>
      ...
      <button
        ...
        className="bg-blue-500 hover:bg-blue-500 disabled:opacity-40 text-white text-sm font-medium rounded px-3 py-2">
